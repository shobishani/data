...
if [ -f /etc/oracle-release ]; then
    ...
    elif grep -q 'Oracle Linux Server release 8.[0-4]' /etc/oracle-release; then
        if [ `uname -m` = "x86_64" ]; then
            OS="OL8-64"
        fi
    fi
    ...
    elif grep -q 'Red Hat Enterprise Linux release 8.[0-3]' /etc/redhat-release || \
        grep -q 'CentOS Linux release 8.[0-3]' /etc/redhat-release; then
	if [ `uname -m` = "x86_64" ]; then
            OS="RHEL8-64"
        fi
    fi
